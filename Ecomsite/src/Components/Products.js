...
const Products = () => {
    ...
    const filterProducts = (typ) => {
        const updatedList = data.filter((x) => x.category === typ);
                        ...
                        <>
                            <div className='col-md-3 mb-4'>
                                <div class="card h-100 text-center p-4" key={product.id}>
                                    <img src={product.image} class="card-img-top" alt={product.title.substring(0, 12)} height="250px" />
                                    <div class="card-body">
                                        <h5 class="card-title mb-0">{product.title.substring(0, 12)}...</h5>
                                        <p class="card-text lead fw-bold">${product.price}</p>
                                        <a href={`/product/${product.id}`} class="btn btn-outline-dark">Buy Now</a>
                                    </div>
                                </div>
            ...
            <div className='container my-5 py-5'>
                <div className='row'>
                    <div className='col-12 mb-5' >
                        <h1 className='display-6 fw-bolder text-center'>Latest Produts</h1> <hr />
                    </div>
                </div>
                ...
                {/* <div>
                    {data.map((item) => {
                        <Product key = {item.id} handClick={handClick} />
                        // console.log(item)
                    })}
                </div> */}
            </div>
